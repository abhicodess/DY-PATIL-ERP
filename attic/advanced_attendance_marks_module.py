"""
═══════════════════════════════════════════════════════════════════════
  ADVANCED ATTENDANCE + MARKS MODULE  — DY Patil ERP
═══════════════════════════════════════════════════════════════════════
  INSTALLATION
  ────────────
  ...
  NO existing routes or templates are changed.

  NEW ROUTES ADDED
  ────────────────
  [ADMIN]
    GET  /admin_att_marks_dashboard        — Advanced dashboard: filter by dept/div/sem
    GET  /admin_student_att_marks/<int:id> — Deep-dive for one student
    GET  /export_att_marks_excel           — Excel: attendance + marks for a dept/div

  [FACULTY]
    GET  /faculty_att_marks                — Faculty sees attendance + marks for their students

  [STUDENT]
    GET  /student_att_marks                — Student sees own combined attendance + marks
═══════════════════════════════════════════════════════════════════════
"""
...
def _student_subject_matrix(student_id, student_name):
    """
    Returns a list of dicts, one per subject, combining attendance and marks.
    {
      subject, att_total, att_present, att_absent, att_late, att_pct,
      att_status,          # 'Good' / 'Average' / 'Low'
      marks_list,          # [{exam_type, marks, total, pct, grade}, ...]
      avg_marks_pct,       # float or None
      best_grade,          # str or 'N/A'
      combined_score       # weighted composite (att 40% + marks 60%)
    }
    ...
    # ── All attendance rows ───────────────────────────────────────────
    att_rows = qry(
        f"SELECT subject, status FROM attendance WHERE {ms}",
        sp,
    )
    # ── All marks rows ────────────────────────────────────────────────
    marks_rows = qry(
        f"SELECT subject, exam_type, marks, total FROM marks WHERE {mms} ORDER BY subject, exam_type",
        msp,
    )
...
def _student_overall_summary(student_id, student_name):
    ...
    att_rows   = qry(f"SELECT status FROM attendance WHERE {ms}", sp)
    marks_rows = qry(f"SELECT marks, total FROM marks WHERE {mms}", msp)
...
@app.route("/admin_att_marks_dashboard")
@login_required("admin")
def admin_att_marks_dashboard():
    """
    Advanced dashboard combining attendance + marks for every student.
    Filter by department, division, year, or semester.
    Shows:
      • Per-student: att%, marks%, combined score, at-risk flag
      • Sortable table with drill-down links
      • Subject-level averages across the filtered group
    """
    ...
    if division: sql += " AND division=?";   params.append(division)
    if year:     sql += " AND year=?";       params.append(year)
    if q:        sql += " AND (name LIKE ? OR roll LIKE ?)"; params += [f"%{q}%", f"%{q}%"]
    sql += " ORDER BY name"
    ...
    for s in students_all:
        ...
        if semester:
            m_rows = qry(
                f"SELECT marks, total FROM marks WHERE {marks_match_student_sql()} AND semester=?",
    ...
    # ── Aggregates for the filtered group ──────────────────
    ...
    # Attendance averages per subject
    att_subj_rows = qry("""
        SELECT a.subject,
               COUNT(*) as total,
    ...
    """.format(dept_clause=f"AND s.department='{dept}'" if dept else ""))
    ...
    sems_list = [r["semester"] for r in qry("SELECT DISTINCT semester FROM marks WHERE semester!='' ORDER BY semester")]

    return render_template("adv_att_marks_dashboard.html",
        rows=rows,
...
@app.route("/admin_student_att_marks/<int:student_id>")
@login_required("admin")
def admin_student_att_marks(student_id):
    """Full attendance + marks breakdown per subject for one student (admin view)."""
    s = qone("SELECT * FROM students WHERE id=?", (student_id,))
    if not s:
        return redirect("/admin_att_marks_dashboard?error=not_found")
    s = dict(s)
    ...
    trend_rows = qry(
        f"SELECT date, subject, status FROM attendance WHERE {ms} ORDER BY date DESC LIMIT 60",
        sp,
    )
    ...
    recent_marks = qry(
        f"SELECT * FROM marks WHERE {mms} ORDER BY date DESC LIMIT 20",
        msp,
    )
...
@app.route("/export_att_marks_excel")
@login_required("admin")
def export_att_marks_excel():
    """Export per-student attendance% + marks% combined to Excel."""
    dept     = request.args.get("dept",     "").strip()
    division = request.args.get("division", "").strip()
    year     = request.args.get("year",     "").strip()
    ...
    wb = Workbook(); ws = wb.active; ws.title = "Attendance+Marks"
...
@app.route("/faculty_att_marks")
@login_required("faculty")
def faculty_att_marks():
    """
    Faculty sees a combined view: for each subject they teach,
    per-student attendance + marks side by side.
    """
    ...
    my_subjects = [r["name"] for r in qry(
        "SELECT name FROM subjects WHERE teacher LIKE ? ORDER BY name",
    ...
    mark_subjects = [r["subject"] for r in qry(
        "SELECT DISTINCT subject FROM marks WHERE faculty_id=? ORDER BY subject", (fid,)
    )]
    ...
    for subj in all_subjects:
        # All students who have attendance OR marks for this subject via this faculty
        student_ids_att = qry(
            "SELECT DISTINCT student_id FROM attendance WHERE subject=? AND student_id IS NOT NULL",
            (subj,),
        )
        student_ids_marks = qry(
            "SELECT DISTINCT student_id FROM marks WHERE subject=? AND faculty_id=? AND student_id IS NOT NULL",
            (subj, fid),
        )
        ...
        for sid in sorted(all_ids):
            s = qone("SELECT id,name,roll,department FROM students WHERE id=?", (sid,))
            if not s:
                continue
            s = dict(s)
            ...
            att_rows = qry(
                "SELECT status FROM attendance WHERE student_id=? AND subject=?",
                (sid, subj),
            )
            ...
            m_rows = qry(
                "SELECT exam_type, marks, total FROM marks WHERE student_id=? AND subject=? AND faculty_id=? ORDER BY exam_type",
                (sid, subj, fid),
            )
            m_list = [{
    ...
    return render_template("faculty_att_marks.html",
...
@app.route("/student_att_marks")
@login_required("student")
def student_att_marks():
    """
    Student's own combined attendance + marks page.
    One card per subject: attendance bar + all exam marks in a mini-table.
    Shows: which subjects are at risk, overall health score.
    """
    student = get_student()
    if not student:
    ...
    # Subjects where student needs to attend more (to reach 75%)
    # Formula: lectures_needed = ceil((0.75 * (total + x) - present) / 0.25)
    shortage = []
    for row in matrix:
        if row["att_pct"] < 75 and row["att_total"] > 0:
            t = row["att_total"]
            p = row["att_present"]
            # min x extra lectures (all present) to cross 75%
            # 0.75*(t+x) <= p+x  =>  x >= (0.75t - p) / 0.25
    ...
    return render_template("student_att_marks.html",
        student=student,
        matrix=matrix,
        summary=summary,
        shortage=shortage,
...
@app.route("/api/student_att_marks_summary/<int:student_id>")
@login_required("admin")
def api_student_att_marks_summary(student_id):
    """JSON: quick summary for a student (used in modal pop-ups)."""
    s = qone("SELECT id,name,roll,department FROM students WHERE id=?", (student_id,))
    if not s:
...
@app.route("/api/att_marks_dept_stats")
@login_required("admin")
def api_att_marks_dept_stats():
    """JSON: department-level attendance + marks averages."""
    stats = []
    for d in DEPARTMENTS:
        students = qry("SELECT id,name FROM students WHERE department=?", (d,))
        att_pcts   = []
        marks_pcts = []
        for s in students:
            summary = _student_overall_summary(s["id"], s["name"])
